...
func _ready():
	...
	print("Can we spawn? %s" % EntityManager.instance.get_entity(instance.ref_id).some()) # calling get_entity will cause the enity manager to start tracking this instance, if it isn't already.
	queue_free()
